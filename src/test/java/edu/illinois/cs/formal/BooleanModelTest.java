...
public class BooleanModelTest
{
	...
	public void test1(){
		...
		Set<Integer> reduced=BooleanModel.TestReduction(ctx, instance);
	...
	public void test2(){
		...
		Context ctx = new Context();
		// input matrix: each row denotes a requirement or coverage element, while each column denotes a test
		int[][] instance = { { 1, 0, 1, 0 }, { 0, 0, 1, 0 },{ 1, 0, 0, 0 }, { 1, 0, 1, 1 } };
		// expected reduced test suite, including only the first and third tests
		Set<Integer> expected=new HashSet<Integer>(Arrays.asList(1, 3));
		// run the reduction and get the actual reduced test suite
		Set<Integer> reduced=BooleanModel.TestReduction(ctx, instance);
		// check if the reduced test set is as expected
		assertEquals(expected, reduced);
